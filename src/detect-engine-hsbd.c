                                     ...
                                     void *tx, uint64_t idx)
{
    ...
    if (htp_state->cfg->decomp_swf_enabled) {
        ...
        if (file_swf_type == FILE_IS_NOT_SWF ||
            file_swf_type == FILE_SWF_NO_COMPRESSION)
        {
            goto skip;
        }

        int r = FileDecompressSWF(&buffer, &buffer_len,
                                  htp_state->cfg->decomp_swf_type,
                                  htp_state->cfg->decomp_swf_decompress_depth,
                                  htp_state->cfg->decomp_swf_compress_depth);
        if (r == FILE_SWF_DECOMP_NOK) {
            SCLogInfo("decompression is not working");
        }
    }
skip:
                                      ...
                                      void *tx, uint64_t tx_id)
{
    ...
    SCLogInfo("htp_state->cfg->decomp_swf_enabled %d", htp_state->cfg->decomp_swf_enabled);
    if (htp_state->cfg->decomp_swf_enabled) {
        int file_swf_type = FileIsFlashFile(buffer, buffer_len);
        if (file_swf_type == FILE_IS_NOT_SWF ||
            file_swf_type == FILE_SWF_NO_COMPRESSION)
        ...
        r = FileDecompressSWF(&buffer, &buffer_len,
                              htp_state->cfg->decomp_swf_type,
                              htp_state->cfg->decomp_swf_decompress_depth,
                              htp_state->cfg->decomp_swf_compress_depth);
        if (r == FILE_SWF_DECOMP_NOK) {
