...
{ 
    ...
    JsonBuilder *jb = CreateEveHeader((Packet *)p, LOG_DIR_PACKET, "arp", NULL, json_output_ctx->eve_ctx);
    if (unlikely(jb == NULL)) {
        return TM_ECODE_OK;
    }    
...
{
    OutputRegisterPacketSubModule(LOGGER_JSON_ARP, "eve-log", "JsonArpLog",
