...
static int ParseCommandLineAfpacket(SCInstance *suri, const char *in_arg)
{
    ...
    if (RunmodeIsUnknown(runmodes) && !RunmodeGetNumber(runmodes)) {
        ...
        if (in_arg) {
            LiveRegisterDevice(in_arg, RUNMODE_AFP_DEV);
            memset(suri->pcap_dev, 0, sizeof(suri->pcap_dev));
...
static int PostConfLoadedSetup(SCInstance *suri)
{
    ...
    PreRunInit(RunmodeGetPrimary(&suri->runmodeslist));
...
int main(int argc, char **argv)
{
    ...
    if (RunmodeGetPrimary(&suricata.runmodeslist) == RUNMODE_DUMP_CONFIG) {
        ConfDump();
        exit(EXIT_SUCCESS);
    }
    ...
    if (ParseInterfacesList(RunmodeGetPrimary(&suricata.runmodeslist), suricata.pcap_dev) != TM_ECODE_OK) {
        exit(EXIT_FAILURE);
    }
    ...
    PreRunPostPrivsDropInit(RunmodeGetPrimary(&suricata.runmodeslist));

    if (RunmodeGetPrimary(&suricata.runmodeslist) == RUNMODE_CONF_TEST){
    ...
    RunModeDispatch(RunmodeGetPrimary(&suricata.runmodeslist), suricata.runmode_custom_mode);
    int i;
    for (i = 1; i < RunmodeGetNumber(&suricata.runmodeslist); i++) {
        if (RunmodeGetCurrent(&suricata.runmodeslist, i) != RUNMODE_UNKNOWN) {
            RunModeDispatch(RunmodeGetCurrent(&suricata.runmodeslist, i), suricata.runmode_custom_mode);
        }
    }

    if (RunmodeGetPrimary(&suricata.runmodeslist) != RUNMODE_UNIX_SOCKET) {
    ...
    PostRunDeinit(RunmodeGetPrimary(&suricata.runmodeslist), &suricata.start_time);
