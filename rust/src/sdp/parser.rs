...
#[inline]
fn is_phone_number_char(b: u8) -> bool {
...
fn parse_phone_number(i: &[u8]) -> IResult<&[u8], String> {
    let (i, phone_number) = preceded(
        tag("p="),
        map_res(take_while(is_phone_number_char), std::str::from_utf8),
    )(i)?;
    let (i, _) = line_ending(i)?;
...
mod tests {
    ...
    #[test]
    fn test_uri_line() {
        let buf: &[u8] = "u=https://www.glongo.it\r\n".as_bytes();

        let (_, u) = parse_uri(buf).expect("parsing failed");
        assert_eq!(u, "https://www.glongo.it");
    }
    ...
    #[test]
    fn test_email_line() {
        let buf: &[u8] = "e=test@sdp.proto\r\n".as_bytes();

        let (_, e) = parse_email(buf).expect("parsing failed");
        assert_eq!(e, "test@sdp.proto");
    }

    #[test]
    fn test_phone_line() {
        let buf: &[u8] = "p=+1 617 555-6011\r\n".as_bytes();

        let (_, p) = parse_phone_number(buf).expect("parsing failed");
        assert_eq!(p, "+1 617 555-6011");
    }
    ...
    fn test_attribute_line() {
        let buf: &[u8] = "a=sendonly\r\na=rtpmap:8 PCMA/8000/1\r\n".as_bytes();
        let (_, a) = parse_attributes(buf).expect("parsing failed");
        assert_eq!(a.first().unwrap(), "sendonly");
